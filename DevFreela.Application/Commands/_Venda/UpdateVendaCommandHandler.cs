...
namespace VisionCar.Application.Commands._Empresa
{
    public class UpdateVendaCommandHandler : IRequestHandler<UpdateVendaCommand, Unit>
    {
        ...
        public async Task<Unit> Handle(UpdateVendaCommand request, CancellationToken cancellationToken)
        {
            ...
            venda.Update(request.Id, request.IdEmpresa, request.IdCliente, request.Valor,request.Caixinha,request.Inicio,request.Fim,request.Data,request.Pago,request.AvisarCliente,request.Excluido,request.Observacao, request.Descricao, request.Placa);
